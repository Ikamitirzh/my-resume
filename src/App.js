...
function App() {
  useEffect(() => {
    ...
    function preload() {
      ...
      this.load.audio("background-music", "assets/music.mp3");
    }

    let player, cursors, dialogueBox, dialogueText, dialogues, dialogueIndex, camera, typingEvent, music;
    let icons = {};
    let currentStage = 0;
    ...
    function create() {
      ...
      // ایجاد شخصیت اصلی
      player = this.physics.add.sprite(100, 450, "amir-idle").setScale(2.5);
      player.setCollideWorldBounds(true);

      music = this.sound.add("background-music", { loop: true });
      music.play();
    ...
    function update() {
      ...
      if (player.x > 600 && currentStage < dialogues.length - 1) {
    ...
    function showDialogue(stage, index) {
      // ریست ایونت تایپ قبلی اگر وجود دارد
      if (typingEvent) {
    ...
    function moveToPreviousStage(scene) {
      // ریست کردن تایپینگ ایونت در صورت فعال بودن
      if (typingEvent) {
        clearInterval(typingEvent);
        typingEvent = null;
      }
